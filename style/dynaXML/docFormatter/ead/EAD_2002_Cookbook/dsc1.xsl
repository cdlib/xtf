...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<!--It formats components that have no container element
	of any type.-->
...
for each level.-->
	...
	<xsl:template name="c02-level-container">
		<xsl:for-each select="did">
			<!--the display is in bold is the component is a
			series, subseries, subcollection or subgroup.-->
			<xsl:choose>
				<xsl:when test="@level='series' or @level='subseries'
				or @level='subcollection' or @level=subgroup'">
				<tr>	
					<td> </td>
