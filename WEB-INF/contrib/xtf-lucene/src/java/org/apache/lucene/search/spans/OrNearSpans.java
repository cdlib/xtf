...
class OrNearSpans implements Spans {
  ...
  private class SpansCell
  {
    ...
    final int doc()               { return spans.doc(); }
    final int start()             { return spans.start(); }
    final int end()               { return spans.end(); }
    final float score()           { return spans.score(); }
  ...
  public OrNearSpans(SpanOrNearQuery query, IndexReader reader, Searcher searcher)
    throws IOException {
    this.query = query;
  ...
  public Explanation explain() throws IOException {
    ...
    Explanation distExpl = new Explanation( 0, "distFactor(sloppyFreq/" + (nClauses+1) + ")" );
    ...
    result.setValue(totalExpl.getValue() *
                    boostExpl.getValue() *
                    combinedFactorsExpl.getValue() );
    return result;
  }
}
