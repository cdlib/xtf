...
import org.apache.lucene.mark.SpanFieldDoc;
import org.apache.lucene.mark.SpanScoreDoc;
...
public class IndexSearcher extends Searcher {
  ...
  public TopDocs search(Query query, Filter filter, final int nDocs)
    ...
    scorer.score(new SpanHitCollector() {
	  ...
	  if (score > 0.0f &&			  // ignore zeroed buckets
        ...
        public final void collect(int doc, float score, FieldSpans spans) {
          if (score > 0.0f &&       // ignore zeroed buckets
            ...
            totalHits[0]++;
            if (hq.size() < nDocs || score >= minScore) {
              hq.insert(new SpanScoreDoc(doc, score, spans));
              minScore = ((ScoreDoc)hq.top()).score; // maintain minScore
            }
          }
        }
    ...
    scorer.score(new SpanHitCollector() {
        ...
        public final void collect(int doc, float score, FieldSpans spans) {
          if (score > 0.0f &&       // ignore zeroed buckets
              (bits==null || bits.get(doc))) {    // skip docs not in bits
            totalHits[0]++;
            hq.insert(new SpanFieldDoc(doc, score, spans));
          }
        }
      });
  ...
  public void search(Query query, Filter filter,
    ...
    if (filter != null) {
      ...
      if (!(results instanceof SpanHitCollector)) {
