...
import org.apache.lucene.mark.FieldSpansContainer;
...
public final class Hits {
  ...
  private final void getMoreDocs(int min) throws IOException {
    ...
    TopDocs topDocs = (sort == null) ? 
            ...
            searcher.search(query, filter, n, sort);
    ...
    for (int i = hitDocs.size(); i < end; i++) {
        HitDoc hitDoc = new HitDoc(scoreDocs[i].score * scoreNorm,
                                   scoreDocs[i].doc);
        if (scoreDocs[i] instanceof FieldSpansContainer )
          hitDoc.fieldMarks = ((FieldSpansContainer)scoreDocs[i]).getFieldSpans();
        hitDocs.addElement(hitDoc);
    }
  }
  ...
  public final Document doc(int n) throws IOException {
    ...
    if (hitDoc.doc == null) {
      ...
      if (hitDoc.fieldMarks != null)
        hitDoc.doc = new SpanDocument(hitDoc.doc, hitDoc.fieldMarks);
    }
