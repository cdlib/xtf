...
public class SpanTermQuery extends SpanQuery {
  ...
  public Spans getSpans(final IndexReader reader, final Searcher searcher) 
      throws IOException 
  {
    // Calculate a score value for this term
    Weight weight = new TermQuery(term).weight(searcher);
    float sum = weight.sumOfSquaredWeights();
    float norm = getSimilarity(searcher).queryNorm(sum);
    weight.normalize(norm);
    final float value = weight.getValue();
    ...
    return new Spans() {
        ...
        public int coordBits() { return coordBits; }
        public Collection allTerms() { return getTerms(); }
