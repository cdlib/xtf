 ...
 * Thus, instead of a huge array indexed by document ID, we keep a list
 * in ID order and binary search it.
 *
 * @author Martin Haye
 */
public class SparseStringComparator implements SortComparatorSource 
{
  ...
  private class SparseComp implements ScoreDocComparator
  {
                ...
                while (termDocs.next()) 
                {
                    ...
                    if( docs.get(key) != null ) {
                        throw new RuntimeException( 
                            "A document has more than one term ('" + 
                            termText + "', '" + (String) docs.get(key) +
                            "') in field " + field );
                    }
    ...
    private Entry findEntry( int docId )
    {
        ...
        int index = Collections.binarySearch( entries, toFind, 
                                              entryComparator );
