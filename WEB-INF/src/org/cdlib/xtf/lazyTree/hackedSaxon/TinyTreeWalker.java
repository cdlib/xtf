...
public class TinyTreeWalker implements PullProvider, SourceLocator 
{
    ...
    throws XPathException 
  {
    if (tree.nodeKind[currentNode] == Type.ELEMENT) 
    {
      ...
      throw new IllegalStateException(
        "getAttributes() called when current event is not ELEMENT_START");
    }
  }
    ...
    throws XPathException 
  {
    if (tree.nodeKind[currentNode] == Type.ELEMENT) 
    {
      if (currentNode == startNode) 
      {
        ...
        int[] decl = TinyElementImpl.getDeclaredNamespaces(tree,
                                                           currentNode,
                                                           nsBuffer);
    ...
    throw new IllegalStateException(
      "getNamespaceDeclarations() called when current event is not ELEMENT_START");
  }
  ...
  public int getTypeAnnotation() 
  {
    if (tree.nodeKind[currentNode] != Type.ELEMENT) {
      throw new IllegalStateException(
        "getTypeAnnotation() called when current event is not ELEMENT_START");
    }
